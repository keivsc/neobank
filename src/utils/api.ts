...
const apiURL = "https://neobank-backend.onrender.com"
...
export async function verifyToken(){
    const res = await post('/user/session', {});
    return res.status === 200;
}

export async function getMyBank(){
    return await get('/bank/me');
}
...
export async function sendMoney(accountNumber:number, amount:number){
    return await put('/bank/transfer', {accountNumber, amount})
}

export async function getNameByAccountNumber(accountNumber:number){
    return await get(`/bank/name/${accountNumber}`);
}

export async function searchNameByAccountNumber(accountNumber:number){
    return await get(`/bank/search/${accountNumber}`);
}

export async function getRequest(accountNumber:number, amount:number){
    return await post(`/bank/request`, {toBankNumber:accountNumber, amount})
}

export async function sendMoneyByRequest(requestId:string, payload:Object, signature:string){
    return await post(`/bank/send/${requestId}`, {payload, signature});
}